...
2. **Judgement pass.** The checker proves files exist, parse, and bridge to a single source of
   ...
   - **Config** — Do the adapters bridge to `AGENTS.md` (import or symlink), or duplicate rules
     ...
     When skills/commands exist: is frontmatter enough for discovery, and do commands stay thin
     entry points? Is `.claude/settings.local.json` gitignored?
   - **Rules** — Is `AGENTS.md` real and specific, or still template stubs? Do any per-tool
     ...
     `~`; only relative or absolute `/…` paths resolve)? If a Tracked findings protocol or
     Progress/plan artifacts are present: is the ledger status authority, is Progress
     chronological only, and would `CLOSED` require exit-gate evidence?
